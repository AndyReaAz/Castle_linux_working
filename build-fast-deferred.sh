...
seed_config()
{
    ...
    if [ -z "$base" ] && [ -f "$ROOT/build-fast/.config" ]; then
        base="$ROOT/build-fast/.config"
        echo "Using validated fast-control config as deferred base: $base"
    fi
    [ -n "$base" ] || die "set KERNEL_BASE_CONFIG, or build the fast control kernel first"
    [ -f "$base" ] || die "KERNEL_BASE_CONFIG does not exist: $base"

    cp "$base" "$OUT/.config"
    echo "Seeded deferred fast-boot config from $base"

    grep -q '^CONFIG_ARCH_AT91=y    grep -q '^CONFIG_MODULES=y$' "$OUT/.config" || die "base config has no module support"
...
case "${1:-build}" in
 ...
 "$OUT/.config" || {
        grep -E '^CONFIG_(ARCH_AT91|SOC_SAMA5D2|MODULES)=' "$OUT/.config" >&2 || true
        die "base config is not AT91; use build-fast/.config rather than a mutable source-tree .config"
    }
    grep -q '^CONFIG_SOC_SAMA5D2=y    grep -q '^CONFIG_MODULES=y$' "$OUT/.config" || die "base config has no module support"
