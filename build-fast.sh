...
seed_config()
{
    ...
    # The 100% new baseline must be reproducible from the repository alone.
    # Start from the upstream Microchip SAMA5 defconfig unless a developer
    # deliberately supplies an alternate seed for diagnostics. Every
    # NextGen-specific requirement is then asserted explicitly below.
    base="${KERNEL_BASE_CONFIG:-$ROOT/arch/arm/configs/sama5_defconfig}"
    [ -f "$base" ] || die "kernel base config does not exist: $base"

    cp "$base" "$OUT/.config"
    echo "Seeded 6.18 config from: $base"

    grep -q '^CONFIG_ARCH_AT91=y
}
...
configure_fast()
{
    ...
    # Hardware/runtime features that used to arrive implicitly through the
    # deployed 6.6 .config are explicit new-baseline inputs now. Keeping these
    # here makes a clean clone produce the same NextGen kernel without any
    # untracked workstation state.
    for sym in \
...
case "$ACTION" in
 ...
 "$OUT/.config" || die "base config is not AT91"
...
case "$ACTION" in
 ...
 "$OUT/.config" || die "base config is not SAMA5D2"
}
